...
def test_calculate(body_part, body, frontal_pixelsPerMetric, lateral_pixelsPerMetric):
    if body_part == "chest":
        frontal_chest = dist.euclidean(body["frontal_points"]["right_chest"], body["frontal_points"]["left_bust"]) / frontal_pixelsPerMetric 
        lateral_chest = dist.euclidean(body["lateral_points"]["chest"], body["lateral_points"]["lower_back"]) / lateral_pixelsPerMetric
        measure = calculate_ellipse_perimeter(frontal_chest/2,lateral_chest/2) 
    elif body_part == "bust":
        frontal_bust = dist.euclidean(body["frontal_points"]["right_bust"], body["frontal_points"]["left_chest"]) / frontal_pixelsPerMetric 
        lateral_bust = dist.euclidean(body["lateral_points"]["lower_bust"], body["lateral_points"]["lower_back"]) / lateral_pixelsPerMetric
        measure = calculate_ellipse_perimeter(frontal_bust/2,lateral_bust/2) 
    return measure
...
def get_measurements(frontal_file, lateral_file, body_height_cm, show = False):
    ...
    #Lateral height:
    #old_lateral_height_in_pixels = dist.euclidean(body["lateral_points"]["heel"], body["lateral_points"]["head"])
    #print("<<<<<<<{}>>>>>>>").format(old_lateral_height_in_pixels)
    lateral_height_in_pixels = dist.euclidean(body["lateral_points"]["heel"],(body["lateral_points"]["heel"][0],body["lateral_points"]["head"][1]))
    ...
    #Test lengths:
    test_chest       = test_calculate("chest",      body, frontal_pixelsPerMetric, lateral_pixelsPerMetric)
    test_bust        = test_calculate("bust",       body, frontal_pixelsPerMetric, lateral_pixelsPerMetric)

    measures = {
        "left_arm": round(left_arm),
        "right_arm": round(right_arm),
        "left_leg": round(left_leg*1.11),
        "right_leg": round(right_leg*1.11),
        "waist": round(waist),
        "hips": round(hips),
        "chest": round(chest),
        "test_chest": round(test_chest),
        "bust": round(bust),
        "test_bust": round(test_bust),
        #"time": "{:.3f}".format(time.time() - t)
        "height": round(body_height_cm),
    ...
    plot_line(body["frontal_points"]["right_waist"],body["frontal_points"]["left_waist"])
    #test front lines
    plot_line(body["frontal_points"]["right_chest"],body["frontal_points"]["left_bust"], color = 'blue')
    plot_line(body["frontal_points"]["left_chest"],body["frontal_points"]["right_bust"], color = 'blue')
    ...
    #test lateral lines
    plot_line(body["lateral_points"]["chest"],body["lateral_points"]["lower_back"], color = 'blue')
    plot_line(body["lateral_points"]["heel"],(body["lateral_points"]["heel"][0],body["lateral_points"]["head"][1]), color = "blue")
...
def obtenerMedidasManual():
    measures1 = get_measurements("test_images/leo_test_front1.jpg","test_images/leo_test_side1.jpg", 181, True)
    measures2 = get_measurements("test_images/leo_test_front2.jpg","test_images/leo_test_side2.jpg", 181, True)
    measures3 = get_measurements("test_images/leo_test_front3.jpg","test_images/leo_test_side3.jpg", 181, True)
    measures4 = get_measurements("test_images/leo_test_front4.jpg","test_images/leo_test_side4.jpg", 181, True)
    print("Medidas:")

    for k, v in measures1.items(): print("{}: {}".format(k,v))
    print("\n")

    for k, v in measures2.items(): print("{}: {}".format(k,v))
    print("\n")

    for k, v in measures3.items(): print("{}: {}".format(k,v))
    print("\n")

    for k, v in measures4.items(): print("{}: {}".format(k,v))
    print("\n")

#obtenerMedidasManual()
